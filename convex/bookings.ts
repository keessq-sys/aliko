    ...
    return Promise.all(
      bookings.map(async b => {
        const [client, plot, payments] = await Promise.all([
          ...
          ctx.db.get(b.plotId),
          ctx.db.query("payments").withIndex("by_booking", q => q.eq("bookingId", b._id)).order("desc").collect(),
        ]);
        const project = plot ? await ctx.db.get(plot.projectId) : null;
...
export const getBookingByReference = query({
  ...
  handler: async (ctx, { reference }) => {
    ...
    if (booking.clientId !== userId && !viewer?.role?.match(/^(ADMIN|AGENT)$/)) {
...
export const confirmPayment = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const duplicate = await ctx.db
      .query("payments")
      .withIndex("by_provider_reference", q => q.eq("provider", "PAYSTACK").eq("providerReference", args.providerReference))
      .first();
    if (duplicate?.status === "SUCCESS") return { booking, isFullyPaid: booking.paymentStatus === "SUCCESS", newlyConfirmed: false };
...
export const cancelBooking = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recentPendingPayment = await ctx.db.query("payments")
      .withIndex("by_booking", q => q.eq("bookingId", args.bookingId))
      .filter(q => q.eq(q.field("status"), "PENDING"))
      .first();
    if (recentPendingPayment && recentPendingPayment.createdAt > Date.now() - 60 * 60 * 1000) {
      throw new Error("Checkout is in progress — wait one hour or contact support before cancelling");
    }
...
export const initializePaystackPayment = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!booking) throw new Error("Booking not found");
    if (booking.clientId !== userId) throw new Error("Forbidden — not your booking");
    if (booking.paymentStatus !== "PENDING" || booking.paidAmount !== 0) throw new Error("This booking is not eligible for a full payment");
...
export const failFlutterwaveAttempt = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (payment && payment.status === "PENDING") {
      await ctx.db.patch(payment._id, { status: "FAILED", metadata: args.metadata, updatedAt: Date.now() });
    }
  },
});
...
export const settleFlutterwavePayment = internalMutation({
  ...
  handler: async (ctx, args) => {
    const payment = await ctx.db.query("payments")
      .withIndex("by_reference", q => q.eq("reference", args.reference))
      .unique();
    if (!payment || payment.provider !== "FLUTTERWAVE") throw new Error("Unknown Flutterwave payment reference");
    const booking = await ctx.db.get(payment.bookingId);
    if (!booking) throw new Error("Booking not found");
    if (payment.status === "SUCCESS") {
      return { booking, isFullyPaid: booking.paymentStatus === "SUCCESS", newlyConfirmed: false };
    }
    const duplicate = await ctx.db.query("payments")
      .withIndex("by_provider_reference", q => q.eq("provider", "FLUTTERWAVE").eq("providerReference", args.providerReference))
      .first();
    if (duplicate && duplicate._id !== payment._id) throw new Error("Flutterwave transaction already processed");
    if (args.currency !== payment.currency || Math.abs(args.amount - payment.amount) > 0.01) {
      throw new Error("Verified payment amount or currency does not match checkout");
    }
    ...
    if (isFullyPaid) await ctx.db.patch(booking.plotId, { status: "SOLD", updatedAt: now });
    return { booking, isFullyPaid, newlyConfirmed: true };
  },
});

async function verifyFlutterwave(ctx: ActionCtx, transactionId: string, expectedReference?: string): Promise<PaymentSettlementResult> {
  ...
  const body = await response.json() as {
    status?: string;
    message?: string;
    data?: { id: number; tx_ref: string; flw_ref?: string; amount: number; charged_amount?: number; currency: string; status: string; payment_type?: string };
  };
  if (!response.ok || body.status !== "success" || !body.data) throw new Error(body.message || "Flutterwave verification failed");
  const transaction = body.data;
  if (transaction.status !== "successful") throw new Error("Flutterwave transaction is not successful");
  if (expectedReference && transaction.tx_ref !== expectedReference) throw new Error("Flutterwave transaction reference mismatch");
  const attempt: { payment: Doc<"payments">; booking: Doc<"bookings"> } | null = await ctx.runQuery(
    internal.bookings.getFlutterwaveAttempt,
    { reference: transaction.tx_ref },
  );
  if (!attempt) throw new Error("Payment attempt not found");
  if (transaction.currency !== attempt.payment.currency || Math.abs(transaction.amount - attempt.payment.amount) > 0.01) {
    throw new Error("Flutterwave transaction amount or currency mismatch");
  }
  const result: PaymentSettlementResult = await ctx.runMutation(internal.bookings.settleFlutterwavePayment, {
    reference: transaction.tx_ref,
...
export const initializeFlutterwavePayment = action({
  ...
  handler: async (ctx, args): Promise<{ checkoutUrl: string; transactionReference: string }> => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Unauthorized");
    const booking: Doc<"bookings"> | null = await ctx.runQuery(internal.bookings.getBookingInternal, { bookingId: args.bookingId });
    if (!booking) throw new Error("Booking not found");
    if (booking.clientId !== userId) throw new Error("Forbidden — not your booking");
    if (booking.paymentStatus !== "PENDING" || booking.paidAmount !== 0) throw new Error("This booking is not eligible for a full payment");
    const secret = process.env.FLUTTERWAVE_SECRET_KEY;
    if (!secret) throw new Error("Flutterwave is not configured");
    if (!/^\S+@\S+\.\S+$/.test(args.email)) throw new Error("A valid receipt email is required");
    const redirect = new URL(args.redirectUrl);
    const isLocal = redirect.hostname === "localhost" || redirect.hostname === "127.0.0.1";
    if (redirect.protocol !== "https:" && !isLocal) throw new Error("Invalid checkout redirect URL");
    const configuredAppUrl = process.env.APP_URL ? new URL(process.env.APP_URL) : null;
    if (configuredAppUrl && !isLocal && redirect.origin !== configuredAppUrl.origin) {
      throw new Error("Checkout redirect URL is not an approved application origin");
    }
    ...
    const response = await fetch("https://api.flutterwave.com/v3/payments", {
      ...
      body: JSON.stringify({
        ...
        meta: { bookingId: String(booking._id), bookingReference: booking.reference },
      }),
    });
    const body = await response.json() as { status?: string; message?: string; data?: { link?: string } };
    if (!response.ok || body.status !== "success" || !body.data?.link) {
      await ctx.runMutation(internal.bookings.failFlutterwaveAttempt, { reference: txRef, metadata: body });
...
export const verifyFlutterwavePayment = action({
  ...
  handler: async (ctx, args): Promise<PaymentSettlementResult> => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Unauthorized");
    const attempt = await ctx.runQuery(internal.bookings.getFlutterwaveAttempt, { reference: args.reference });
    if (!attempt || attempt.booking.clientId !== userId) throw new Error("Payment attempt not found");
...
export const processFlutterwaveWebhook = internalAction({
  args: { transactionId: v.string(), reference: v.string() },
  handler: async (ctx, args) => verifyFlutterwave(ctx, args.transactionId, args.reference),
});
